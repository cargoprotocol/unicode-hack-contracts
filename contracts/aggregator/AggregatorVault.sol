...
contract AggregatorVault is IAggregatorVault, IRebalancerType, ERC20Permit, ReentrancyGuard {
    ...
    function deposit(uint256 _token0Amount, uint256 _token1Amount) external override {
        ...
        uint256 liquidity = _deposit(_token0Amount, _token1Amount, address(this));
        emit AggregatorVaultDeposit(liquidity);
    }

    function withdraw(uint256 _liquidity, address _to) external override {
        (uint256 amount0, uint256 amount1) = _withdraw(_liquidity, _to, address(this));
